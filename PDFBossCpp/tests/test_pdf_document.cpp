...
using pdfboss::PdfDocument;

namespace {

fs::path fixture()
{
    return fs::path(PDFBOSS_TEST_FIXTURE);
}
...
{
    std::vector<pdfboss::TextBlock> blocks;
...
TEST_CASE("fixture opens and reports its shape", "[pdf]")
{
    ...
    const pdfboss::Rect bounds = doc.page_bounds(0);
    // A4 in points.
...
TEST_CASE("search is case-insensitive, as page.search_for is", "[pdf]")
{
    ...
    std::vector<pdfboss::Rect> lower;
    std::vector<pdfboss::Rect> upper;
...
TEST_CASE("words come back split on whitespace with sane boxes", "[pdf]")
{
    ...
    const auto has = [&words](const std::string& text) {
        return std::any_of(words.begin(), words.end(),
                           [&text](const pdfboss::Word& w) { return w.text == text; });
    };
    CHECK(has("quick"));
    CHECK(has("brown"));
    CHECK(has("Introduction"));

    for (const pdfboss::Word& word : words) {
...
TEST_CASE("outline flattens to titles and 1-based pages", "[pdf]")
{
    ...
    const std::vector<pdfboss::OutlineItem> items = doc.outline();
...
{
    const fs::path copy =
        fs::temp_directory_path() / "pdfboss_highlight_roundtrip.pdf";
    std::error_code ec;
    fs::remove(copy, ec);
...

    {
        ...
        const pdfboss::Rect& box = words[0].rect;
