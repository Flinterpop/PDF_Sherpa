...
public:
    ScopedAppData()
        : dir_(fs::temp_directory_path() /
               ("pdfboss_cfg_" + std::to_string(_getpid()) + "_" +
...
TEST_CASE("last_pages prunes the least recently read past the cap", "[config][lru]")
{
    ...
    for (std::size_t i = 0; i < pdfboss::kMaxRememberedPages + 1; ++i) {
    ...
    const nlohmann::ordered_json after =
        nlohmann::ordered_json::parse(appdata.read_raw());
    CHECK(after["last_pages"].size() == pdfboss::kMaxRememberedPages);
    ...
    CHECK(reloaded.last_page(
                  "C:/docs/file" +
                  std::to_string(pdfboss::kMaxRememberedPages) + ".pdf")
              .has_value());
}
...
TEST_CASE("favorites cap at the same limit as the Python app", "[config]")
{
    ...
    for (std::size_t i = 0; i < pdfboss::kMaxFavorites + 5; ++i) {
        many.push_back("fav" + std::to_string(i) + ".pdf");
    }
    REQUIRE(config.save_favorites(many));
    CHECK(config.favorites().size() == pdfboss::kMaxFavorites);

    Config reloaded;
    reloaded.load();
    CHECK(reloaded.favorites().size() == pdfboss::kMaxFavorites);
...
TEST_CASE("roots are capped", "[config][roots]")
{
    ...
    for (std::size_t i = 0; i < pdfboss::kMaxRoots + 3; ++i) {
        many.push_back(Root{"root" + std::to_string(i), "C:\\r" + std::to_string(i)});
    }
    REQUIRE(config.save_roots(many));
    CHECK(config.roots().size() == pdfboss::kMaxRoots);

    Config reloaded;
    reloaded.load();
    CHECK(reloaded.roots().size() == pdfboss::kMaxRoots);
